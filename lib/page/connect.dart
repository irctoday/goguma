...
class _ConnectPageState extends State<ConnectPage> {
	...
	Widget build(BuildContext context) {
			...
			return Scaffold(
				...
				body: Container(padding: EdgeInsets.all(10), child: Column(children: [
					ListTile(
						...
						title: Text('In order to have an optimal IRC experience (message history, notifications when highlighted, multiple networks, sharing files over chat), you will need an IRC bouncer that will stay connected to IRC for you.'),
					),
					ListTile(
						leading: Icon(Icons.check),
						title: Text('You can use Goguma without an IRC bouncer and connect directly to a single IRC network, but you will miss these key features.'),
					),
					ListTile(
						leading: Icon(Icons.check),
					...
					ListTile(
						leading: Icon(Icons.check),
						title: Text('You can change accounts at any time by signing out in the app options.'),
					),
					...
					Spacer(),
					Container(padding: EdgeInsets.symmetric(vertical: 10), child: OutlinedButton(
						onPressed: () {
							setState(() {
								_tutorial = false;
								_ircToday = false;
						...
						child: Text('I have a bouncer account'),
					)),
					Container(padding: EdgeInsets.symmetric(vertical: 10), child: OutlinedButton(
						onPressed: () {
							setState(() {
						...
						child: Text('I wish to connect to a single network (limited features)'),
					)),
				])),
			);
		}
		...
		if (!_ircToday) {
			children.add(Container(padding: EdgeInsets.symmetric(vertical: 10), child: OutlinedButton(
				onPressed: () {
					launchUrl(Uri.parse('$ircTodayUrl/log-in-goguma'));
				},
				child: Text('I have an IRC Today account'),
			)));
		}
		children.add(Container(padding: EdgeInsets.symmetric(vertical: 10), child: OutlinedButton.icon(
			...
			label: Text(_ircToday ? 'I do not have an IRC Today account' : 'I do not have an account'),
		)));
